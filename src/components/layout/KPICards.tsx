...
export default function KPICards({ indicators }: { indicators: Indicators }) {
  ...
  const kpis: KpiDef[] = [
    ...
    { key: 'canceladas', label: 'Canceladas', icon: XCircle, value: indicators.canceladas, color: 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300', active: filters.status.includes('CANCELADA'), onClick: () => openWithStatus(['CANCELADA']) },

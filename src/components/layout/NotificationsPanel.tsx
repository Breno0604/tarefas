...
export default function NotificationsPanel({ open, onOpenChange }: any) {
  const { state, dispatch } = useStore()
  const navigate = useNavigate()
  const ref = useRef<HTMLDivElement | null>(null)
  const reminders = state.reminders || []
  const unread = reminders.filter((r: any) => !r.read).length
  useDismissable(ref, () => onOpenChange(false), open)

  const openTask = (reminder: any) => {
        ...
        >
          ...
          <div className="max-h-[380px] overflow-y-auto">
            {reminders.length === 0 ? (
              ...
              reminders.map((r: any) => {
                const cfg = (TYPE_ICON as any)[r.type] || TYPE_ICON.due
                const Icon = cfg.icon
                return (
                  <button
                    key={r.id}

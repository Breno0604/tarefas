...
export default function CommandPalette({ open, onClose }: any) {
  ...
  const results = useMemo(() => {
    ...
    return {
      tasks,
      projects,
      pages: PAGE_ACTIONS.filter((p: any) => p.label.toLowerCase().includes(q))
    }
  }, [query, state])
  ...
  const flat = useMemo(() => {
    ...
    if (results.tasks.length) {
      ...
      results.tasks.forEach((t: any) => items.push({ type: 'task', data: t }))
    }
    if (results.projects.length) {
      items.push({ type: 'header', label: 'Projetos' })
      results.projects.forEach((p: any) => items.push({ type: 'project', data: p }))
    }
    if (results.pages.length) {
      items.push({ type: 'header', label: 'Navegação' })
      results.pages.forEach((p: any) => items.push({ type: 'page', data: p }))
    }
    return items
  }, [results])
  ...
  const onKeyDown = (e: any) => {
    if (e.key === 'ArrowDown') {
      e.preventDefault()
      setActive((a: any) => Math.min(a + 1, flat.length - 1))

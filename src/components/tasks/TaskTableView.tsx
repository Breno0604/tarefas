...
export default function TaskTableView({ tasks, selected, onToggleAll, onToggleSelect, onSort, sortKey, onOpenTask, onEditTask, onChange, onDeleteTask, onToggleFavorite, onDuplicateTask, onToggleDone, onCancelTask }: any) {
  const { state } = useStore()
  const { show } = useContextMenu()
  const [confirm, setConfirm] = useState(null)

  const allSelected = tasks.length > 0 && tasks.every((t: any) => selected.has(t.id))
  const someSelected = tasks.some((t: any) => selected.has(t.id))
  ...
  return (
    <div className="card-base overflow-hidden relative">
      ...
      <ConfirmDialog
        ...
        message={`Excluir "${(confirm as any)?.title}"? Você poderá desfazer em seguida.`}
      />
    </div>
  )
}

...
export default function TaskCard({
  ...
  return (
    <>
      ...
      >
        <div className={`h-1 w-full ${(PRIORITY_BAR as any)[task.priority]}`} />
        <div className="p-3">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3">
              {task.subtasks.length > 0 && (
                <span
                  ...
                  aria-label={`${task.subtasks.filter((s: any) => s.done).length} de ${task.subtasks.length} subtarefas concluídas`}
                >
                  <ListChecks size={12} />
              ...
              {task.recurrence && task.recurrence !== 'none' && (
                ...
                >
                  <Repeat size={12} />
                  {(RECURRENCE as any)[task.recurrence]?.label?.replace('Todo ', '').replace('Toda ', '').replace('Todos os ', '').replace('Toda semana', 'semanal').replace('Todo mês', 'mensal') || task.recurrence}
                </span>
              )}
            </div>
          )}

...
const CtxMenuContext = React.createContext<any>(null)
...
export function ContextMenuProvider({ children }: any) {
  ...
  const show = useCallback((e: any, items: any[]) => {
    e.preventDefault()
    e.stopPropagation()
  ...
  const handleKeyDown = useCallback(
    (e: any) => {
      if (!menu) return
      const items = menu.items.filter((i: any) => i.type !== 'divider' && !i.disabled)
      if (items.length === 0) return

      switch (e.key) {
        case 'ArrowDown': {
          e.preventDefault()
          setFocusIndex((prev: any) => (prev + 1) % items.length)
          break
        }
        case 'ArrowUp': {
          e.preventDefault()
          setFocusIndex((prev: any) => (prev - 1 + items.length) % items.length)
          break
        }
        case 'Home': {
          e.preventDefault()
  ...
  useEffect(() => {
    ...
    const btn = buttons[focusIndex] as HTMLElement | undefined
    btn?.focus()
  }, [focusIndex])
          ...
          >
            {menu.items.map((item, i) => {
              ...
              const actionableItems = menu.items.filter(
                (it: any) => it.type !== 'divider' && !it.disabled
              )

...
function TaskRow({ task, project, onToggle, onOpen }) {
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="flex items-center gap-2">
            <span className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ backgroundColor: PRIORITY[task.priority]?.hex || '#94a3b8' }} />
...
function Section({ icon: Icon, iconClass, title, count, accent, children, empty }) {
  return (
...
function TodayPage() {
  ...
  const openTask = (id) => navigate(`/tarefas?task=${id}`)
  ...
  const projectOf = (t) => state.projects.find((p) => p.id === t.projectId)
  const rowProps = (t) => ({
    key: t.id,
    task: t,
    project: projectOf(t),
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      >
        {sections.overdue.map((t) => <TaskRow {...rowProps(t)} />)}
      </Section>
      ...
      >
        {sections.today.map((t) => <TaskRow {...rowProps(t)} />)}
      </Section>
      ...
      >
        {sections.upcoming.map((t) => <TaskRow {...rowProps(t)} />)}
      </Section>
      ...
      {sections.undated.length > 0 && (
        <Section
          ...
          empty={null}
        >
          {sections.undated.slice(0, 10).map((t) => <TaskRow {...rowProps(t)} />)}
        </Section>
      )}
    </div>
  )

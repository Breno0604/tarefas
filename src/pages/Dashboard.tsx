...
function StatCard({ icon: Icon, iconClass, label, value, sub, trend, trendUp, onClick }: { icon: any; iconClass: string; label: string; value: string | number; sub?: string; trend?: string; trendUp?: boolean; onClick?: () => void }) {
  return (
...
function navigateWithClearFilters(navigate: any, path: string) {
      ...
      <div className="card-base p-5">
        ...
        {today.agenda.length === 0 ? (
          ...
          <ul className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2">
            {today.agenda.map((t: any) => {
              const project = state.projects.find((p: any) => p.id === t.projectId)

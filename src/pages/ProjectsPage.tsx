...
function ProjectsPage() {
  ...
  useEffect(() => {
    if (projParam && state.projects.some((p: any) => p.id === projParam)) {
  ...
  const stats = useMemo(() => {
    ...
    state.projects.forEach((p: any) => {
      ...
      const overdue = tasks.filter((t: any) => isOverdue(t.dueDate, t.status)).length
  ...
  const selectedProject = selectedId ? state.projects.find((p: any) => p.id === selectedId) : null
  const selectedTasks = selectedId
    ? state.tasks.filter((t: any) => t.projectId === selectedId)
    : []
    ...
    <div className="space-y-6">
      ...
      >
        <div className="space-y-4">
          <Input label="Nome do projeto" placeholder="Ex.: Aplicativo de Delivery" value={form.name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm((f: any) => ({ ...f, name: e.target.value }))} />
          <Input label="Descrição" placeholder="Resumo do objetivo do projeto" value={form.description} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm((f: any) => ({ ...f, description: e.target.value }))} />
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input type="date" value={form.due} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm((f: any) => ({ ...f, due: e.target.value }))} />
            </div>
          </div>
        </div>
      </Modal>
          ...
          <div className="space-y-5">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="grid flex-1 grid-cols-3 gap-3">
                <div className="rounded-lg bg-slate-50 p-3 text-center dark:bg-slate-800/60">
                  <p className="text-lg font-extrabold text-slate-800 dark:text-slate-100">{(stats as any)[selectedProject.id].total}</p>
                  <p className="text-[11px] font-semibold text-slate-400">Tarefas</p>
                </div>
                <div className="rounded-lg bg-slate-50 p-3 text-center dark:bg-slate-800/60">
                  <p className="text-lg font-extrabold text-emerald-600 dark:text-emerald-400">{(stats as any)[selectedProject.id].done}</p>
                  <p className="text-[11px] font-semibold text-slate-400">Concluídas</p>
                </div>
                <div className="rounded-lg bg-slate-50 p-3 text-center dark:bg-slate-800/60">
                  <p className="text-lg font-extrabold text-red-600 dark:text-red-400">{(stats as any)[selectedProject.id].overdue}</p>
            ...
            <div>
              <div className="flex items-center justify-between text-sm">
                <span className="font-semibold text-slate-500 dark:text-slate-400">Progresso</span>
                <span className="font-bold text-slate-800 dark:text-slate-100">{(stats as any)[selectedProject.id].pct}%</span>
              </div>
              <ProgressBar value={(stats as any)[selectedProject.id].pct} color={selectedProject.color} className="mt-2" />
            </div>

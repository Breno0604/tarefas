...
export default function TasksPage() {
  ...
  useEffect(() => {
    ...
    const onToggleFav = () => f.setFavoritesOnly((v: boolean) => !v)
  ...
  const changeTask = (patch: any, taskId: any) => {
  ...
  const requestCancel = (task: any) => {
    setCancelTarget(task)
  }
  const confirmCancel = (reason: any) => {
    if (!cancelTarget) return
    dispatch({
      type: 'CANCEL_TASK',
      taskId: (cancelTarget as any).id,
      reason: reason || null
    })
  ...
  const bulkDeleteAll = () => {
    bulkDeleteWithUndo({ dispatch, toast, taskIds: Array.from(f.selected) as string[] })
    f.setSelected(new Set())
    setBulkDelete(false)
  }
  ...
  const allTags = useMemo(() => {
    const set = new Set()
    state.tasks.forEach((t: any) => (t.tags || []).forEach((tag: any) => set.add(tag)))
    return [...(set as Set<string>)].sort((a, b) => a.localeCompare(b, 'pt-BR'))
  }, [state.tasks])
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          onClick={() => f.setFavoritesOnly((v: boolean) => !v)}
          className={`shrink-0 inline-flex h-9 w-9 items-center justify-center rounded-lg border transition ${
            f.favoritesOnly
        ...
        <>
          ...
          {view === 'list' && (
            <div className="card-base overflow-hidden">
              <ul>
                {f.pageTasks.map((task: any) => (
                  <TaskListItem
                    key={task.id}
                    task={task}
                    project={state.projects.find((p: any) => p.id === task.projectId)}
          ...
          {view === 'table' && (
            <React.Suspense fallback={<div className="card-base p-8 text-center text-sm text-slate-400">Carregando...</div>}>
              <TaskTableView
                ...
                onToggleAll={() =>
                  f.setSelected((s: any) =>
                    s.size === f.pageTasks.length ? new Set() : new Set(f.pageTasks.map((t: any) => t.id))
                  )
                }
          ...
          {view === 'kanban' && (
            ...
            <KanbanView
              tasks={f.filtered}
              onOpenTask={openTask}
              onEditTask={(task: any, defaults: any) => (task ? openEdit(task) : openCreate(defaults))}
              onChange={changeTask}
              onDeleteTask={deleteTask}
              onToggleFavorite={toggleFavorite}
              onDuplicateTask={duplicateTask}

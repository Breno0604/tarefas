import type { TaskStatus, TaskPriority, RecurrenceType } from '../types'

export const STATUS: Record<TaskStatus, { key: TaskStatus; label: string; hex: string; badge: string; dot: string; bar: string }> = {
  todo: {
    key: 'todo',
    label: 'A fazer',
    hex: '#94a3b8',
...
export const PRIORITY: Record<TaskPriority, { key: TaskPriority; label: string; hex: string; rank: number }> = {
...
export const SORT_OPTIONS: Array<{ key: string; label: string }> = [
...
export const RECURRENCE: Record<RecurrenceType, { key: RecurrenceType; label: string }> = {
 ...
 */
export function nextRecurrenceDate(dueDate: string | null, recurrence: RecurrenceType): string | null {
